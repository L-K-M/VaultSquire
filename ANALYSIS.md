# VaultSquire Analysis (ANALYSIS.md)

The consolidated, forward-looking analysis of the VaultSquire codebase,
merged from the GLM review (`glm.md`). Items addressed during the review
pass are marked **✅ Done** with their PR; everything else is open work.
...
Findings carry a **risk** note because the authoring environment is Linux and
cannot compile Swift (per `AGENTS.md`, this environment verifies only
documentation and repository hygiene). Every Swift change therefore still
needs macOS/Xcode verification, which is the documented norm for this repo.
...
### B3. Search query survives a vault-scope change — ✅ Done (PR #64)
`VaultBrowserView` previously cleared only `selection` on scope change. It now
clears `query` too.
File: `VaultSquire/Features/Vault/VaultBrowserView.swift`.

### B4. Stale selection can outlive a filtered-out item — ✅ Done (PR #64)
`onChange(of: query)` now drops the selection when the filtered list no longer
contains it, so the detail pane and highlighted row track the visible list.
File: `VaultSquire/Features/Vault/VaultBrowserView.swift`.
...
### B6. `spacedCode` only spaces 6-digit codes — ✅ Done (PR #54)
`spacedCode` now groups 6/7/8-digit codes (3+3, 3+4, 4+4).
File: `VaultSquire/Features/Vault/VaultItemDetailView.swift`.
...
### M3. Keyboard-navigable Quick Search (↑/↓ to move, ↩ to open, ⌘↩ copy password, etc.) — ✅ Done (PR #51)
The panel now supports ↑/↓ selection (clamped) and ↩ opens the selection or
first result. ⌘↩ copy-password remains open (needs a per-row secret hydration
path).

### M4. Item icons in Quick Search results — ✅ Done (PR #51)
Results carry a deterministic monogram badge (same hue formula as the main
list). Live site icons are deliberately not fetched from the panel; see N1 for
the safe way to add live TOTP/icons later.
File: `VaultSquire/Features/QuickSearch/QuickSearchView.swift`.
...
### V5. TOTP row has no progress affordance — ✅ Done (PR #54)
The TOTP row now carries a shrinking circular arc with the seconds remaining at
its centre, turning amber in the final five seconds.
File: `VaultSquire/Features/Vault/VaultItemDetailView.swift`.
...
### V9. Quick Search panel is visually bare — ✅ Done (PR #51)
The panel now shows per-result monogram badges, a keyboard-navigable selection
with a hint footer. Recents-when-empty (N9) remains open.
File: `VaultSquire/Features/QuickSearch/QuickSearchView.swift`.
...
### V12. No empty state for the main list when search yields nothing — ✅ Done (PR #64)
The browser now shows a `ContentUnavailableView` ("No matches" / "No items")
...
- **#51 — Quick Search: icons, keyboard navigation, hint footer.** Addresses
  M3, M4, V9 (and the B10-adjacent UX). Each result carries a deterministic
  monogram badge; ↑/↓ move a clamped selection; ↩ opens the selection or first
  result; a hint footer teaches the gestures. The monogram is drawn by a
  self-contained view because the panel is hosted outside the main scene's
  environment and cannot reach the shared `SiteIconStore`, and a transient
  panel should not fire favicon requests per row. Adds unit tests for selection
  clamping and open-on-enter.
- **#54 — Item detail: TOTP countdown ring and digit grouping.** Addresses
  V5, B6, A6. Replaces the plain `Xs` countdown with a shrinking arc that
  carries the seconds remaining at its centre and turns amber in the final five
  seconds; `spacedCode` now groups 6/7/8-digit codes.
- **#64 — Browser: clear search on scope change, drop stale selection, empty
  state.** Addresses B3, B4, V12. Scope change now clears the query; a filter
  that hides the selected row drops the selection; an empty filtered list shows
  a `ContentUnavailableView`.

All three are additive, view-layer changes. None touch crypto, persistence,
secret flow, the clipboard, or add a new network surface. Authored under Linux,
so each PR still needs macOS/Xcode verification (the documented norm).

### Deliberately deferred (open backlog)

These are the higher-value items that need design or macOS verification beyond
a view-layer change, and are the basis for future work:

- **Password generator** (G8 / M1) — random + memorable + pronounceable, with a
  live strength meter.
- **Global, system-wide Quick Search hotkey** (M2 / B10) — the current shortcut
  is app-level only; a real global hotkey has accessibility/prompt implications
  to reconcile with `SECURITY_AND_TESTING.md`.
- **Account removal / reconfigure UI** (G1) — `AccountDescriptorStore.remove`
  exists but no UI calls it.
- **Multi-account Vaultwarden** (G2).
- **Edit/create for non-login types** (G3) — cards, identities, secure notes.
- **Custom-field editing** (G4).
- **Trash / Archive view with restore** (G5 / M7).
- **Favorites scope + star indicator** (G6 / M6).
- **Sort options** (G7) — recent / modified / folder / type.
- **Right-click context menu and row copy actions** (G9 / U1) — copy username,
  password, TOTP, open site.
- **Keyboard copy/open shortcuts** (G10 / U3).
- **Auto-lock / clipboard timing controls in Settings** (G11).
- **Fuzzy / subsequence search with ranking** (M11) — and centralize the two
  duplicate `matches(_:)` helpers.
- **Perf rework of `AppModel.items`** (P1 / P2) — sort once when items are
  produced rather than on every body access; this changes state flow and must be
  verified against the performance fixtures on a Mac.
- **Locked-shell semantic/adaptive colours** (G12 / V1).
- **Detail grouped-card layout and label casing** (V2 / V3 / A1).
- **Detail header balance** (V4).
- **Length-aware secret mask** (V6).
- **Toolbar status pill** (V7).
- **Per-category icon tints** (A5), **spring transitions** (A3), **spacing
  scale** (A4).
- **Copy confirmation feedback** (U7).
- **Novel ideas** (§8): safe live-TOTP in rows (N1, needs design — seeds must
  stay out of projections), vault identity colours (N2), command palette (N3),
  privacy-preserving breach check (N4, opt-in only), memorable generator (N5),
  TOTP QR import (N6), in-memory clipboard history (N7), open-state window
  accent (N8), recents-when-empty in Quick Search (N9), drag-to-fill (N10),
  menu-bar mini-app (N11 / M5), delight on Touch ID setup (N12).
- **Remaining bugs**: B1 (auto-lock granularity), B2 (lingering typed password),
  B5 (TOTP countdown ceil), B7 (global `isWriting` dismiss coupling), B8 (Quick
  Search snapshot staleness), B9 (icon fetch failure permanent for session),
  B10 ("global" hotkey is app-level — also M2).

Everything in sections 1–9 that is not marked **✅ Done** above is still open and
is preserved verbatim for future work.
