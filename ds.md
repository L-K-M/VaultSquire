...
### B4. Partial CLI refreshes silently replace a complete offline snapshot (covered)
...
longer has the vaults that failed, with no marker that it is incomplete.

`Covered by the parallel PR fix/complete-cli-snapshots` ("Fail partial CLI snapshot
refresh publication"), which fails the refresh closed and preserves the last
complete offline snapshot.
...
### B6. Auto-lock is active but invisible and mis-documented (covered)
...
`Covered by the parallel PRs fix/honest-settings-copy` (caption truthfulness) and
`feat/autolock-preference` (a full inactivity picker bound to the controller's key).
...
`Covered by the parallel PR fix/detail-copy-boundaries`, which confines text
selection to non-secret fields. The related copy-confirmation feedback is in
`PR: feature/copy-confirmation` (this pass).
...
### F2. No auto-lock control in Settings (covered)

Covered by the parallel PR `feat/autolock-preference` (an inactivity picker bound to
the key `AutoLockController` already reads), together with `fix/honest-settings-copy`
for the caption. See B6.
...
|---|---|---|
| B1, B2 | `fix/browser-selection-correctness` (PR #38) | Quick Search hand-off widens scope for group scopes too; scope-change no longer wipes a handed-off selection; selection reconciled when query or items remove it |
| B3 | `fix/reopen-main-window` (PR #39) | `Window` → `WindowGroup` + `applicationShouldHandleReopen` fallback |
| B5 | `feature/archive-confirmation` (PR #43) | Confirmation dialog before archiving |
| F1, U4 | `feature/password-generator` (PR #42) | Offline generator + dice/popover UI + password reveal toggle + tests |
| V7 | `feature/copy-confirmation` (PR #45) | Transient "Copied" state on detail copy buttons |
| P3, U1 (badge), U2 | `feature/quick-search-polish` (PR #44) | Result cap (100), globally sorted merged results, per-vault source badge, tests |

Covered by the parallel review session instead (drafts at review time): B4 →
`fix/complete-cli-snapshots`; B6/F2 → `feat/autolock-preference` and
`fix/honest-settings-copy`; S1 → `fix/detail-copy-boundaries`; B8 →
`fix/clear-icons-on-vault-lock`.
